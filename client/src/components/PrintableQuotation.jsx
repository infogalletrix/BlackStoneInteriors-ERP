    ...
    <div className="grid grid-cols-2 gap-3 my-2">
      <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-2.5 shadow-xs">
        ...
        <div className="space-y-0.5 text-[10px]">
          <div className="text-[13px] font-black text-slate-900 leading-snug">{safe.customer || safe.clientName || "Valued Client"}</div>
          {safe.organizationName && <div className="text-[10.5px] font-bold text-slate-700">{safe.organizationName}</div>}
          {(safe.address || safe.clientAddress) && <div className="text-[10px] text-slate-600 leading-normal">📍 {safe.address || safe.clientAddress}</div>}
          <div className="flex flex-wrap gap-x-3 text-[9.5px] text-slate-700 font-semibold pt-0.5">
            {(safe.mobileNo || safe.phone) && <span>📞 +91 {safe.mobileNo || safe.phone}</span>}
            {(safe.emailId || safe.email) && <span>✉️ {safe.emailId || safe.email}</span>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-2.5 shadow-xs">
        ...
        <div className="grid grid-cols-[90px_auto] gap-y-1 text-[10px]">
          {(safe.projectTitle || safe.project) && (
            <>
              <span className="text-slate-500 font-bold">Project Name:</span>
              <span className="font-black text-slate-900 text-[10.5px]">{safe.projectTitle || safe.project}</span>
            </>
          )}
