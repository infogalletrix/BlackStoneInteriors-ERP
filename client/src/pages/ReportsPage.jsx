  ...
  const fetchReportData = async (reportId) => {
    ...
    try {
      switch(reportId) {
        ...
        case 'invoices': { const d = await (await fetch('/api/finance/invoices')).json(); title="Invoice Register"; header=["Date","Invoice No","Client","Total","Status"]; rows=d.map(e=>[new Date(e.date).toLocaleDateString('en-IN'),e.invoiceNo,e.clientName,e.total || 0,e.status||"Paid"]); break; }
