      ...
      <div className="p-6 page-wrapper min-h-screen font-sans relative">
        <div className="flex justify-between items-center mb-6">
          <button
            ...
            className="flex items-center gap-2 text-muted hover:text-accent font-bold text-sm transition"
          >
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="px-4 py-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl text-sm font-bold text-themed hover:border-accent hover:text-accent transition flex items-center gap-2 shadow-sm"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="themed-card rounded-[32px] p-8 text-center shadow-sm">
              <div className="w-24 h-24 bg-accent/10 text-accent rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-accent/20 shadow-md">
                <User size={40} />
              </div>
              <h2 className="text-2xl font-black text-themed mb-1">{selectedEmployee.name}</h2>
              <p className="text-accent font-bold text-xs tracking-widest uppercase mb-4">
            ...
            <div className="themed-card rounded-[32px] p-6 shadow-sm">
              <h3 className="font-black text-themed text-sm mb-4 flex items-center gap-2">
                <Phone size={16} className="text-accent" /> Contact &amp; Details
              </h3>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="themed-card rounded-[32px] overflow-hidden shadow-sm">
              <div className="p-6 px-8 border-b border-[var(--border-color)] flex items-center gap-3 font-black text-themed">
                <CreditCard size={18} className="text-accent" /> Payment History
              </div>
  ...
  return (
    <div className="p-4 md:p-6 page-wrapper">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-3 relative z-50">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="bg-accent text-white px-4 py-1.5 rounded-xl font-bold hover:bg-accent-hover shadow-md flex items-center gap-2 transition ml-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard title="Monthly Payroll" value={`₹${totalMonthlyPayroll.toLocaleString()}`} color="text-accent" icon={Briefcase} />
      </div>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          ...
          <div className="flex bg-white/10 p-1 rounded-xl">
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${viewMode === 'list' ? 'bg-[var(--bg-card)] shadow-sm text-accent' : 'text-muted hover:text-themed'}`}
            >
              <List size={16} />
            </button>
            <button
              onClick={() => setViewMode("grid")}
              className={`p-1.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-[var(--bg-card)] shadow-sm text-accent' : 'text-muted hover:text-themed'}`}
            >
              <LayoutGrid size={16} />
            </button>
          </div>
                ...
                {filteredEmployees.map((emp) => (
                  <tr key={emp.id} className="group themed-row">
                    <td className="px-6 py-4">
                      <button onClick={() => setSelectedEmployee(emp)} className="block font-black text-themed text-sm hover:text-accent transition text-left">
                        {emp.name}
                      </button>
          ...
          {filteredEmployees.map((emp) => (
            <div key={emp.id} className="themed-card rounded-[24px] p-5 shadow-sm hover:shadow-md transition flex flex-col h-full relative group">
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-[var(--bg-card)] p-1 rounded-lg shadow-sm border border-[var(--border-color)]">
                <button onClick={() => handleEdit(emp)} className="text-muted hover:text-accent p-1">
                  <Edit2 size={14} />
                </button>
                <button
                    onClick={() => {
              ...
              <div className="w-16 h-16 bg-accent/10 text-accent rounded-full flex items-center justify-center mb-4 cursor-pointer" onClick={() => setSelectedEmployee(emp)}>
                <User size={24} />
              </div>
              <h3 className="font-black text-themed text-lg cursor-pointer hover:text-accent transition" onClick={() => setSelectedEmployee(emp)}>
                {emp.name}
              </h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <button
              type="submit"
              className="w-full bg-accent text-white py-4 rounded-xl font-black text-base mt-8 hover:bg-accent-hover shadow-md transition"
            >
