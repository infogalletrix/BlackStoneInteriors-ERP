 ...
 *
 * Fixes "Safari can't open this page" on macOS (Intel/M1) by detecting Safari desktop
 * and using jsPDF's built-in .save() instead of blob URL anchor navigation, which Safari
 * desktop refuses to handle (it tries to open blob: URL as a page and fails).
 *
 * Strategy:
 * 1. iOS/iPadOS  → Web Share API (native Save to Files sheet)
 * 2. macOS Safari → pdf.save() via jsPDF (uses data URI internally, bypasses blob URL issue)
 * 3. All others  → Standard blob URL <a download> anchor (Chrome, Firefox, Edge, Android)
 */
export const savePdfFile = async (pdf, fileName = "Quotation.pdf") => {
  ...
  // Detect iOS / iPadOS (touch-based Apple devices)
  const isAppleMobile =
  ...
  // Detect macOS Safari desktop (not Chrome/Edge which also include "Safari" in UA)
  // Safari UA contains "Safari" but NOT "Chrome" or "Chromium" or "Edg"
  const isSafariDesktop =
    !isAppleMobile &&
    /^((?!chrome|chromium|edg|android).)*safari/i.test(ua) &&
    /Macintosh|MacIntel|MacPPC|Mac68K/.test(ua);

  // --- 1. iOS / iPadOS: Use Web Share API for native Save to Files ---
  if (isAppleMobile && typeof File !== "undefined") {
    try {
      ...
      console.warn("navigator.share failed, falling back to pdf.save():", shareErr);
    }
    // Fallback: jsPDF .save() for older iOS where share API failed
    pdf.save(cleanFileName);
    return;
  }

  // --- 2. macOS Safari desktop: Use jsPDF's .save() to avoid blob URL navigation error ---
  if (isSafariDesktop) {
    // pdf.save() uses a data URI download internally which Safari handles correctly
    pdf.save(cleanFileName);
    return;
  }

  // --- 3. All other browsers (Chrome, Firefox, Edge, Android): Standard blob URL anchor ---
  const blob = pdf.output("blob");
  ...
  link.rel = "noopener";
  link.target = "_self"; // _blank can trigger Safari popup blocker

  document.body.appendChild(link);
  link.click();

  // Keep the blob URL alive long enough for the browser to process the download
  setTimeout(() => {
    try {
